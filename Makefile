.PHONY: tests help install venv lint isort tcheck build commit-checks prepare pypibuild pypipush docker
SHELL := /usr/bin/bash
.ONESHELL:
...
PKG_SOURCES := flickr_immich_k8s_sync_operator/*
...
dist/flickr_immich_k8s_sync_operator-$(VERSION).tar.gz dist/flickr_immich_k8s_sync_operator-$(VERSION)-py3-none-any.whl dist/.touchfile: $(PKG_SOURCES) $(VENV_DEPS) pyproject.toml
...
pypibuild: venv dist/flickr_immich_k8s_sync_operator-$(VERSION).tar.gz dist/flickr_immich_k8s_sync_operator-$(VERSION)-py3-none-any.whl

dist/.touchfile_push: dist/flickr_immich_k8s_sync_operator-$(VERSION).tar.gz dist/flickr_immich_k8s_sync_operator-$(VERSION)-py3-none-any.whl
	@$(venv_activated)
...
DOCKER_IMAGE := flickr-immich-k8s-sync-operator
